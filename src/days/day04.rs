    ...
    fn part_1(input: &Self::ParsedData) -> Self::Part1Result {
        ...
        for i in 0..input.len() as i32 {
            for j in 0..input[0].len() as i32 {
                if *get_char(input, i, j).unwrap_or(&' ') == 'X' {
                    for x in -1..2 as i32 {
                        for y in -1..2 as i32 {
                            if *get_char(input, i + x, j + y).unwrap_or(&' ') == 'M'
                                && *get_char(input, i + 2 * x, j + 2 * y).unwrap_or(&' ') == 'A'
                                && *get_char(input, i + 3 * x, j + 3 * y).unwrap_or(&' ') == 'S'
                            {
                                count += 1;
                            }
                        }
...
fn get_char(input: &Vec<Vec<char>>, i: i32, j: i32) -> Option<&char> {
    ...
    input.get(i as usize)?.get(j as usize)
}
